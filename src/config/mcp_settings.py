...
嵌套在 :class:`config.settings.AppSettings` 的 ``mcp_settings`` 字段中加载；环境变量前缀为 ``PTAGENT_MCP_SETTINGS__``（例如 ``PTAGENT_MCP_SETTINGS__ENDPOINT``）。
...
def get_mcp_settings() -> MCPSettings:
    """返回当前应用配置中的 MCP 段（与 ``get_settings().mcp_settings`` 等价）。"""

    from .settings import get_settings

    return get_settings().mcp_settings
