"""Real Foldseek plus UniProt FASTA sequence-channel smoke test.

This test verifies the complete two-channel path:

    AlphaFold model -> Foldseek structure neighbor
    UniProt FASTA -> sequence.kmer neighbor
    -> fused candidate -> KG candidate edge -> CTD hypothesis -> report

Foldseek, the downloaded UniProt FASTA catalogue, and local CTD facts are
real. Accession-to-gene resolution remains a small static TSV fixture until a
real UniProt MCP is available. The generated hypothesis is still subject to
deep-search validation and is not a biological finding.
"""
...
from pkg.graph import Direction, EdgeType, GraphEdge, InMemoryGraphStore, NodeLabel, NodeRef  # noqa: E402
from pkg.protein_db import parse_fasta  # noqa: E402
from pkg.retrieval.providers import SequenceNeighborProvider  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="Q09178",
        help="Target expected from both Foldseek and sequence.kmer.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Local UniProt FASTA catalogue used by the sequence channel.",
    )
    parser.add_argument(
        "--sequence-version",
...
def main() -> None:
    args = parse_args()
    exp_id = args.experiment_id or _experiment_id("exp_fs_sequence_hypothesis_smoke_")
    ...
    missing_accessions = sorted(required_accessions - set(sequences))
    if missing_accessions:
        raise SystemExit(
            f"sequence FASTA is missing required accessions: {missing_accessions}"
        )
    ...
    if expected_sequence_hit is None:
        raise SystemExit(
            "expected target was not recalled by sequence.kmer; "
            "increase --top-k or inspect the FASTA catalogue"
        )
    ...
    structure_settings = StructureSettings(
        ...
        exclude_self=True,
        version=f"alphafold-v{latest_version}-sequence-smoke-{exp_id}",
    )
    ...
    result = run_downstream_pipeline(
        ...
        config=DownstreamPipelineConfig(
            ...
            pipeline_version="foldseek-sequence-hypothesis-real-smoke",
            top_k=args.top_k,
            ...
            graph_store=graph,
            # This test intentionally stops before deep-search.
    ...
    expected_two_channel_candidate = bool(
        fused_candidate
        and {"structure", "sequence"}.issubset(fused_candidate.support_channels)
    )
    ...
    if not result.completed:
        raise SystemExit(1)
    if not structure_evidence or not sequence_runs or not sequence_evidence:
        raise SystemExit("one or more neighbor channels produced no persisted evidence")
    if expected_sequence_hit is None or not expected_sequence_evidence:
        raise SystemExit("expected target is missing from persisted sequence evidence")
    if not expected_two_channel_candidate:
        raise SystemExit("expected target did not receive structure and sequence support")
